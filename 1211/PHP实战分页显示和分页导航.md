...
数据分页其实就是SQL构造语句的Limit使用，其有两种形式:`SELECT * FROM user LIMIT 0,10;`和`SELECT * FROM USER LIMIT 10 OFFSET 0;`。若是`LIMIT 0,10`则偏移量是0，10是记录条数，而是使用`LIMIT 10 OFFSET 0`表示意思是一样的，即若LIMIT后面只有一个数字，则表示取得记录条数。

关于偏移量的计算是`offset=(page-1)*num;`。`page`是当前页的页码，即是`index.php?p=2`中2。num则表示每页要显示的记录条数。
