...
> **关于文档片断再补充:** 后来老师在作业批改的评语中举例子解释了，如ul...li结构，一般页面中已经存在ul元素了，li是根据计算动态添加的，此时内存直接组装就无法实现，添加到ul就是真实添加到了页面。此时需要在 **内存中有个虚拟的ul元素代替页面中ul角色** ，这时在内存中组装就不影响页面了，这个就是文档片断的由来，在挂载文档片断时，它将被页面中元素真实替代，其子元素就真实挂载到页面中了，效率非常好。
>
>   - **内存中** ，**文档片断虚拟了页面中元素** ，在内存中完成后代元素组装
>   - **页面中** ，**真实元素替代文档片断**，接收文档片断的后代元素
>   - 若创建**父元素在内存中**，则**内存直接组装**和**文档片断组装**效果是一样的。
